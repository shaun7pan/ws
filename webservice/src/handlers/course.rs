...
pub async fn get_courses_for_teacher(
    ...
    get_courses_for_teacher_db(&app_state.db, teacher_id).await.map(|courses|HttpResponse::Ok().json(courses))
}
...
pub async fn update_course_details(
    ...
    let (teacher_id, course_id) = params.into_inner();
    update_course_details_db(&app_state.db, teacher_id, course_id, update_course.try_into()?).await.map(|course|HttpResponse::Ok().json(course))
}
...
mod tests{
    ...
    async fn update_course_success(){
        ...
        let update_param = web::Json(update_couse);
    ...
    async fn delete_course_failure(){
        ...
        let params: web::Path<(i32,i32)> = web::Path::from((1,120));
